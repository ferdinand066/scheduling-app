...
export const find = async (req: Request, h: ResponseToolkit) => {
  ...
  try {
    ...
    // Build TypeORM FindManyOptions from query params
    const filter: any = {};
    ...
    if (query.filter) {
      ...
      for (const [field, conditions] of Object.entries(query.filter)) {
        if (typeof conditions === 'object' && conditions !== null) {
          const cond = conditions as any;
          
          // Handle gte and lte together (Between)
          if (cond.gte && cond.lte) {
            filter.where[field] = Between(cond.gte, cond.lte);
          } else if (cond.gte) {
            filter.where[field] = MoreThanOrEqual(cond.gte);
          } else if (cond.lte) {
            filter.where[field] = LessThanOrEqual(cond.lte);
          }
        } else {
          // Direct equality filter
          filter.where[field] = conditions;
        }
      }
    }
